...
pub(crate) use tokens::ident_token::IdentToken;
...
mod whitespace {
    use super::{code_points::LF, filtered::Filtered};
...
// mod array_ring_buffer;
mod array_vec;

mod filtered {
    use konst::{string::Chars, try_opt};

    use super::tokens::{hex_digit::HexDigit, hex_digits::HexDigits};

    use super::{code_points::LF, is_ident_code_point, whitespace::Whitespace};

    /// https://drafts.csswg.org/css-syntax-3/#css-filter-code-points
    pub(super) struct Filtered<'a>(pub Chars<'a>);

    #[derive(Debug, Clone, Copy)]
    pub(super) struct FilteredChar(char);
    ...
    impl Filtered<'_> {
        ...
        pub const fn next_two_are_whitespace(&self) -> bool {
            let stream = self.copy();
            if let Some((_, stream)) = Whitespace::consume(stream) {
                Whitespace::consume(stream).is_some()
            } else {
                false
            }
        }
    ...
    impl<const N: usize> FilteredCharVec<N> {
        ...
        pub const fn crop_and_fit<const M: usize>(
            self,
            remove_first_n: usize,
        ) -> FilteredCharVec<M> {
            let mut res = FilteredCharVec::EMPTY;
        ...
        pub const fn truncate_and_fit<const M: usize>(
            self,
            keep_first_n: usize,
        ) -> FilteredCharVec<M> {
